...
from greatx.nn.layers import Sequential, DGConv, activations
...
class DGC(nn.Module):
    ...
    >>> # DGC with very deep architectures, each layer has elu as activation function
    ...
    def __init__(self,
        ...
        for i, (hid, act) in enumerate(zip(hids, acts)):
            if i == 0:
                conv.append(DGConv(in_channels,
                                   hid,
                                   bias=bias,
                                   K=K,
                                   t=t,
