...
target_class = 0
predict = trainer.predict(data).argmax(-1)
count = (predict == target_class).int().sum().item()
print(f"{count/data.num_nodes:.2%} of nodes are classified as class "
      f"{target_class} before attack")
...
for t in tqdm(range(data.num_nodes)):
    t_class = trainer.predict(attacker.data(t), mask=t).argmax(-1)
    if t_class == target_class:
        count += 1

print(f"{count/data.num_nodes:.2%} of nodes are classified as class "
      f"{target_class} after backdoor attack")
