    ...
    * Please remember to call :meth:`reset` before each attack.
    ...
    def compute_cooccurrence_constraint(self, nodes):
        ...
        for n in nodes:
            ...
            scores = np.array(
                [idegs[nnz == ix].sum() for ix in range(num_feats)])
    ...
    def compute_logits(self):
        return (self.adj_norm @ self.adj_norm @ self.modified_feat
    ...
    def feature_scores(self):
        ...
        X_influencers[self.influence_nodes] = self.modified_feat[
    ...
    def get_attacker_nodes(self, n=5, add_additional_nodes=False):
        ...
        assert n < self.modified_adj.shape[0] - \
            1, "number of influencers cannot be >= number of nodes!"
        ...
        # The new A_hat_square_uv values that we would get
        # if we removed the edge from u to each of the neighbors, respectively
        ...
        if len(neighbors) >= n:
            ...
            if add_additional_nodes:
                ...
                # Compute the set of possible additional influencers, i.e.,
                # all nodes except the ones that are already connected to u.
                poss_add_infl = np.setdiff1d(
                    np.setdiff1d(np.arange(self.modified_adj.shape[0]),
                                 neighbors), self.target)
                n_possible_additional = len(poss_add_infl)
                n_additional_attackers = n - len(neighbors)
                possible_edges = np.column_stack(
                    (np.tile(self.target, n_possible_additional),
                     poss_add_infl)).astype("int32")

                # Compute the struct_scores for all possible additional
                # influencers, and choose the one with the best struct score.
    ...
    def get_candidate_edges(self, n_influencers):
        # Potential edges are all edges from any attacker to any other node,
        # except the respective attacker itself or the node being attacked.
        target = self.target
        N = self.num_nodes
    ...
    def attack(self, target, *, target_label=None, num_budgets=None,
               n_influencers=5, direct_attack=True, structure_attack=True,
               feature_attack=False, ll_constraint=True, ll_cutoff=0.004,
               disable=False):
        ...
        for it in tqdm(range(self.num_budgets), desc='Perturbing graph...',
                       disable=disable):

            best_edge_score = best_feature_score = 0
            if structure_attack:
                # Do not consider edges that, if removed,
                # result in singleton edges in the graph.
                if not self._allow_singleton:
                    candidate_edges = singleton_filter(candidate_edges,
                                                       self.modified_adj)

                if ll_constraint:
                    candidate_edges = likelihood_filter(
                        candidate_edges,
                ...
                # Compute struct scores for each potential edge
                struct_scores = self.structure_score(a_hat_uv_new,
                                                     self.compute_XW())
            ...
            if feature_attack:
                # Compute feature scores for each feature perturbation
