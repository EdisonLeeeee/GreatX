...
def overlap(edge_index1: Tensor, edge_index2: Tensor, 
    ...
    Parameters
    ----------
    edge_index1 (Tensor) : edges indicies of a graph
        a graph
    edge_index2 (Tensor) : edges indicies of another graph
        another graph
    ...
    if on == 'edge':
        ...
        raise ValueError(f"It currently only supports overlapping on `edge` or `node`, but got {on}.")
