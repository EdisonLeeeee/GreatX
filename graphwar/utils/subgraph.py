...
subgraph = namedtuple('ego_graph', ['nodes', 'edges'])


__all__ = ['k_hop_subgraph', 'ego_graph']
...
def k_hop_subgraph(node_idx, num_hops, edge_index, relabel_nodes=False,
    ...
    Parameters
    ----------
    node_idx (int, list, tuple or :obj:`torch.Tensor`)
        The central node(s).
    num_hops: (int): 
...
def ego_graph(adj_matrix: sp.csr_matrix, targets: Union[int, list], hops: int = 1):
    ...
    return subgraph(nodes=np.asarray(targets),
