...
from graphwar.defense.data_level import SVDPurification
...
# ============ SVD Defense =============================================
defense_g = SVDPurification()(attacker.g())
...
# ============ After evasion Attack (with SVD) =========================
logs = trainer.evaluate(defense_g, y_test, splits.test_nodes)
print(f"After evasion attack with SVD\n {logs}")
...
# ============ After poisoning Attack (with SVD) ======================
model = GCN(num_feats, num_classes)
trainer = Trainer(model, device=device)
trainer.fit(defense_g, y_train, splits.train_nodes)
logs = trainer.evaluate(defense_g, y_test, splits.test_nodes)
print(f"After poisoning attack with SVD\n {logs}")
